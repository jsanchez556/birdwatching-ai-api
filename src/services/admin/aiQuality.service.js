import aiQualityRepository from '../../db/repositories/admin/aiQuality.repository.js';
...
function safeProvenance(snapshot) {
  ...
  const safe = {
    sourceType: typeof snapshot.sourceType === 'string' ? snapshot.sourceType : null,
    sourceArtifactId: typeof snapshot.sourceArtifactId === 'string'
      ? snapshot.sourceArtifactId
      : null,
  };
  for (const field of fields) {
    safe[field] = typeof provenance[field] === 'string' ? provenance[field] : null;
  }
  return safe;
}
...
function observationsFromSnapshots(snapshots = []) {
  return snapshots
    .filter((snapshot) => (
      snapshot?.evaluationType === 'portfolio_regression'
      && snapshot?.validRealPipelineOutputs === true
      && snapshot?.evidenceClass === 'real_pipeline_output'
    ))
    .flatMap((snapshot) => {
...
class AiQualityService {
  ...
  async getQualitySummary(range) {
    ...
    const snapshots = await this.repository.getEvaluationSnapshots();
    const validSnapshots = snapshots.filter((snapshot) => (
      snapshot?.evaluationType === 'portfolio_regression'
      && snapshot?.validRealPipelineOutputs === true
      && snapshot?.evidenceClass === 'real_pipeline_output'
    ));
    const observations = observationsFromSnapshots(validSnapshots);
    const latest = [...validSnapshots].sort((left, right) => (
      String(right.timestamp || '').localeCompare(String(left.timestamp || ''))
    ))[0];
    const selfTests = snapshots.filter((snapshot) => (
      snapshot?.evaluationType === 'scorer_self_test'
    ...
    return {
      ...
      qualityStatus: validSnapshots.length ? 'available' : 'unavailable',
      qualitySource: validSnapshots.length ? 'real_pipeline_output' : null,
      unavailableReason: validSnapshots.length
        ? null
        : 'No valid portfolio regression artifact from real pipeline outputs is available.',
      provenance: safeProvenance(latest),
      scorerSelfTest: {
        label: 'Synthetic scorer self-test — not model or RAG quality',
        includedInQualityMetrics: false,
        availableInConfiguredArtifact: selfTests.length > 0,
      },
