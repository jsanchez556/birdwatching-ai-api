...
| Context assembly | Runtime context items are assembled in memory from existing chat, RAG, tool, and application sources. The default long-term-memory adapter stores and returns nothing. | Context packages are request-scoped and are not added to a new durable store. Existing source retention rules still apply. | Selected model-facing content flows to OpenAI; only aggregate counts, token estimates, durations, bounded reason codes, model/task keys, and degraded-source names are eligible for metrics. | Provenance and metrics exclude raw content, preferences, document text, tool payloads, PII, and raw user/conversation identifiers. Visitor assembly does not retrieve authenticated long-term memory. |
