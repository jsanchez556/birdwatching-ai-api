...
`conversations.metadata` when an exchange is saved.
...
- `getLatestByUserId(userId)` finds the latest owned conversation by `last_message_at DESC NULLS LAST, created_at DESC`, then the service loads messages and returns persisted chat-level metadata.
